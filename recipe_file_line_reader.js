...
function writeRecipesToDatabase(recipes){
  ...
  db.serialize(function() {
     ...
     sqlString = "CREATE TABLE IF NOT EXISTS recipes (id INTEGER PRIMARY KEY, recipe_name TEXT, spices TEXT, category TEXT, description TEXT, directions TEXT)";
     ...
     */
     var stmt = db.prepare("INSERT INTO recipes (recipe_name,spices,category, description, directions) VALUES (?,?,?,?,?)");
     for (var i = 0; i < recipes.length; i++) {
   	    recipe = recipes[i];
        stmt.run(recipe.recipe_name, recipe.spices, recipe.category, recipe.description, recipe.directions);
     }
     stmt.finalize();
  
     db.each("SELECT id, recipe_name, spices, category, description, directions FROM recipes", function(err, row) {
         console.log(row.id + ": " +
             		 row.recipe_name + " " +
					 row.spices + " " + 
					 row.category + " " +
    ...
    function(line, last) { 
		...
		if(isOpeningTag(str)){
		   ...
		   if(str === '</recipe_name>') {
			   ...
			   //if(dataString == null){recipe.category=""}
			   recipe.category = dataString;
		   }
		    else if(str === '</description>'){
			   //if(dataString == null){recipe.description=""}
			   recipe.description = dataString;
		   }
		    else if(str === '</directions>'){
			   //if(dataString == null){recipe.directions=""}
