...
exports.songDetails = function(request, response){
		...
		db.all(sql, function(err, rows){
		  ...
		  song.individualBars = [];
		  let str = '';
		  //remove double bar lines and final bar line
		  for(let i=0; i<song.bars.length; i++){
			  if(song.bars.substr(i,2) === '||'){				  
			  }
			  else if(i>0 && song.bars.substr(i-1,2) === '|]'){
			  }
			  else {
				 str = str.concat(song.bars.substr(i,1));
			  }
		  }
		  song.bars = str;
		  str = '';
		  //parse into individual bars
		  for(let i=0; i<song.bars.length; i++){
			  if(song.bars.substr(i,1) === '|'){
				  song.individualBars.push(str);
				  str = '';
			  }
			  else{
				str = str.concat(song.bars.substr(i,1));  
